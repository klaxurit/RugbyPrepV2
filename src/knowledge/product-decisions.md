...
| Rail J-2 runtime | Date séance ∈ [J-2, match] (jours calendaires) → `variant: light` + `maxBlocks` ≤ 2, bumps volume coupés. Pas de rewrite mother. Sans event match → rien. | — | Fenêtre 48 h depuis midi (rate le jeudi) / light hors calendrier réel |
