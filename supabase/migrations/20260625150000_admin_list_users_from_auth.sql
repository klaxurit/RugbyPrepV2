-- Liste tous les comptes auth.users (pas seulement ceux avec une ligne profiles).
...
comment on function public.admin_list_users_page(int, int, text) is
  'Service-role : tous les comptes auth.users paginés, avec profil optionnel et séances de la semaine ISO (Europe/Paris).';
