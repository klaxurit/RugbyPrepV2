 ...
 */
export function WarmupBlock({ block, number, state, expanded, onToggle }: WarmupBlockProps) {
  return (
      ...
      {expanded && block.exercises.length > 0 && (
        <ul className="flex flex-col gap-2 px-1">
          {block.exercises.map((exo, i) => (
            <WarmupItem key={i} number={i + 1} exo={exo} />
          ))}
        </ul>
      )}
    </div>
...
function WarmupItem({ number, exo }: WarmupItemProps) {
  return (
    <li className="flex items-center gap-3 rounded-xl border border-paper-deep bg-app px-3.5 py-2.5">
      ...
      <span className="flex-1 text-[13px] font-semibold text-fg">{exo.name}</span>
